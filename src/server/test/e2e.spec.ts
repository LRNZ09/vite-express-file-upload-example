...
describe('E2E', () => {
	...
	it('should upload a file', async () => {
		...
		formData.append(
			'file',
			new Blob(['Hello from LRNZ09!'], { type: 'text/plain' }),
			filename,
		);
		const promise = await fetch(
			`http://localhost:${TESTING_PORT}/api/upload-single`,
			{
				method: 'POST',
				body: formData,
			},
		...
		const content = new TextDecoder().decode(
			new Uint8Array(
				new Uint8Array(readFileSync(`${TESTING_UPLOADS_DIR}/${filename}`)),
			),
		);
		expect(content).toBe('Hello from LRNZ09!');
	});
	...
	it('should upload a "chunked" file (only one chunk)', async () => {
		...
		const promise = await fetch(
			`http://localhost:${TESTING_PORT}/api/upload-chunk`,
			{
				method: 'POST',
				body: formData,
			},
		...
		expect(data).toHaveProperty(
			'message',
			'Chunked file uploaded successfully',
		);
	...
	it('should upload a "chunked" file (multiple chunks)', async () => {
		...
		const content = 'Hello from LRNZ09!';
		...
		expect(fileContent).toBe('Hello from LRNZ09!');
	});
});
